...
func GetEnforcerWithFile() *casbin.Enforcer {
	var err error
	once.Do(func() {
		enforcer, err = gcasbin.New(app.Viper())
		if err != nil {
			panic(err)
		}
	})
...
func NewCasbinService(domain string,
	repo repositories.ISysCasbinRuleRepo) *CasbinService {
	serv := new(CasbinService)
	serv.dom = domain
	e := app.CasbinEnforcer()
	a := casbinAdapter.NewCasbinAdapter(repo)
	e.SetAdapter(a)
	e.ClearPolicy()
	e.LoadPolicy()
	serv.Enforcer = e
...
```go
// AddUserRoleRule 添加用户授权规则
func (s SysUserService) AddUserRoleRule(roleIds []int64, userId int64) error {
	// var err error
	serv := NewCasbinService("admin", repositories.NewSysCasbinRuleRepository(s.repo.DB()))
...
```go
    ...
    // 文件权限策略
    fileEnforcer := services.GetEnforcerWithFile()
    fileHasPerm, _ := fileEnforcer.Enforce(fmt.Sprintf("%d", userid), "admin", path, method)

    // 数据库的权限策略
    hasPerm, _ := casbinServ.Enforcer.Enforce(fmt.Sprintf("%d", userid), "admin", path, method)
